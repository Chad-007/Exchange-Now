...
import { useParams } from "react-router-dom";
    ...
    const PageStyles = () => (
        ...
        .left-sidebar, .right-sidebar { display: flex; flex-direction: column; gap: 0.75rem; }
        .chart-container { grid-column: 2; }
        ...
        .order-book-body { display: flex; flex-direction: column; flex-grow: 1; }
        ...
        .depth-bar { position: absolute; top: 0; bottom: 0; opacity: 0.12; }
        ...
        .asks-list, .bids-list { display: flex; flex-direction: column; }
        .asks-list { flex-direction: column-reverse; }

        /* Trade History */
        .trade-history-container { min-height: 250px; }
        .trade-history-body { flex-grow: 1; overflow-y: auto; }
        .trade-history-row { display: grid; grid-template-columns: 1fr 1fr 1fr; font-size: 0.75rem; padding: 3px 1rem; }
    ...
    useEffect(() => {
        ...
        api.get(`/markets/${symbol}/trades`).then((res) => setTrades(res.data.slice(0, 15)));
        api.get(`/markets/${symbol}/ticker`).then((res) => setTicker(res.data));
        
        connectSocket(
        symbol,
        (tradeData: Trade) => { setTrades((prev) => [tradeData, ...prev].slice(0, 15)); },
    ...
    const handlePlaceOrder = async () => {
        ...
        try {
        // NOTE: The backend needs to be able to handle a `type` field.
        // For a market order, price can be omitted.
        const payload: any = {
            symbol,
            side: orderSide,
            type: orderType,
        ...
        const response = await api.post("http://152.42.158.207:4002/api/orders", payload);
                ...
                <div className="order-book-body">
                    <div className="asks-list">
                        {asksWithCum.reverse().map((ask, i) => (
                            <div className="order-book-row" key={`ask-${i}`} onClick={() => handleRowClick(ask.price)}>
                                ...
                                <span className="total-col">{ask.cumulative.toFixed(4)}</span>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="bids-list">
                        {bidsWithCum.map((bid, i) => (
                            <div className="order-book-row" key={`bid-${i}`} onClick={() => handleRowClick(bid.price)}>
                                ...
                                <span className="total-col">{bid.cumulative.toFixed(4)}</span>
                            </div>
                        ))}
                    </div>
                </div>
    ...
    const renderPlaceOrderForm = () => {
        ...
        return (
            <div className="trade-card order-form-container">
