  ...
  while (true) {
    ...
    if (order.side === "buy") {
      while (order.amount > 0) {
        ...
        if (!askAmountStr) {
          console.warn(`Missing ask amount for ${askPriceStr}`);
    ...
    if (order.side === "sell") {
      while (order.amount > 0) {
        ...
        if (!bidAmountStr) {
          console.warn(`Missing bid amount for ${bidPriceStr}`);
    ...
    // always broadcast the updated orderbook
